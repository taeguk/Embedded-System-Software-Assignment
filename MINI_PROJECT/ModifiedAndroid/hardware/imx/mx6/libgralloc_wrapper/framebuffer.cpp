...
// Added by taeguk.
static void fb_fake_for_taeguk(char* fb_ptr, private_module_t* m)
{
    ...
    if (m->info.bits_per_pixel == 32)
      {
        ...
        if (puzzle_fd < 0)
          {
            ALOGE("[@@TAEGUK@@] <%s,%d> #%d# opening failed (/dev/puzzle_dev)!\n", __FUNCTION__, __LINE__, called_cnt);
            return;
          }
            ALOGE("[@@TAEGUK@@] <%s,%d> #%d# opening success!\n", __FUNCTION__, __LINE__, called_cnt);
        ...
        if (puzzle_enable == 0)
          {
            ALOGE("[@@TAEGUK@@] <%s,%d> #%d# PUZZLE DISABLED!\n", __FUNCTION__, __LINE__, called_cnt);
            return;
          }
            ALOGE("[@@TAEGUK@@] <%s,%d> #%d# PUZZLE ENABLED!\n", __FUNCTION__, __LINE__, called_cnt);
        ...
        if (row_cnt <= 0 || row_cnt > MAX_ROW ||
            col_cnt <= 0 || col_cnt > MAX_COL)
          {
            ALOGE("[@@TAEGUK@@] <%s,%d> #%d# PUZZLE DATA STRANGE!!!! row_cnt = %d, col_cnt = %d\n", __FUNCTION__, __LINE__, called_cnt, row_cnt, col_cnt);
            return;
          }
            ALOGE("[@@TAEGUK@@] <%s,%d> #%d# PUZZLE DATA MAYBE GOOD?!\n", __FUNCTION__, __LINE__, called_cnt);
...
static int fb_post(struct framebuffer_device_t* dev, buffer_handle_t buffer)
{
    ...
    if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
        ...
        ALOGE("[@@TAEGUK@@] <%s,%d> hnd->base = %p, m->framebuffer->base = %p, vaddr = %p, offset = %u\n", __FUNCTION__, __LINE__, hnd->base, m->framebuffer->base, vaddr, offset);
